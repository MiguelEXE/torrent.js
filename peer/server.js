...
class PeerConnection extends EventEmitter{
    ...
    /**
     * Peer (client) info hash sent on handshake
     */
    get infoHash(){
    ...
    constructor(socket){
        ...
        this.#connection.on("error", error => {
            this.emit("warn", `net.Socket emitted a error. ${error.stack}`);
        });
        this.#connection.on("data", data => {
            ...
            if(!this.#handshaked){
                ...
                this.#info.infoHash = remote_info_hash;
                const remote_peer_id = buf.readBuffer(20);
                this.#info.peerId = remote_peer_id;
                this.#peerSentHandshake = true;
                return this.emit("connectionAttempt", {
                    peerId: remote_peer_id,
                    infoHash: remote_info_hash
                });
            }
            this.#onMessage(buf);
        });
    ...
    /**
     * Sends a `keepAlive` opcode. Used to test is the client is properly responding to the messages
     */
    keepAlive(){
     ...
     * @param {Buffer} infoHash Info hash to send. It needs to be equal of the info hash sent by the peer on handshake
     * @param {Buffer} peerId Our peer id
     */
    acceptConnection(infoHash, peerId){
        assert.ok(!this.#connection.closed, new Error("connection closed"));
        assert.ok(!this.#handshaked, new Error("already handshaked"));
        assert.ok(this.#peerSentHandshake, new Error("a handshake attempt needs to occur"));

        const handshake = peerUtils.createHandshake(infoHash, peerId);
...
class PeerServerOptions{
    /**
     * Hostname to host the server
...
class PeerServer extends EventEmitter{
    ...
    /**
     * Initializes a peer server
     * @param {PeerServerOptions} options Peer server host options
     */
    constructor(options){
        super();
        assert.deepStrictEqual(typeof options.port, "number", new TypeError("options.port is not a number"));
        if(options.hostname === undefined){
            options.hostname = "127.0.0.1";
        }else{
            assert.deepStrictEqual(typeof options.hostname, "string", new TypeError("options.hostname is not a string"));
            validateHostname(options.hostname);
        }
        this.#server = net.createServer(socket => {
            const connection = new PeerConnection(socket);
            this.#clients.push(connection);
            socket.once("close", () => {
                const index = this.#clients.findIndex(c => c === connection);
                if(index > -1){
